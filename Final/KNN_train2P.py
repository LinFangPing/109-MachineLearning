...
Plat2PX = np.array(P2PlatformPosition)[:,0][:, np.newaxis]
Plat2PX_next = Plat2PX[1:,:]
instruct = (Plat2PX_next - Plat2PX[0:len(Plat2PX_next)])/5     #已知的結果
...
print(Plat2PX[0:-1].shape)
print(Ball.shape)
print(speed.shape)
x = np.hstack((Plat2PX[0:-1],Ball,speed)) #輸入
y = instruct
print(x.shape)
print(y.shape)
